...
internal static class AccountingMapper
{
    ...
    public static AccountDto ToDto(Account x) => new(x.Id, x.Code, x.NameAr, x.NameEn, x.AccountType, x.NormalBalance, x.ParentAccountId, x.IsPostingAccount, x.IsActive);
    public static CostCenterDto ToDto(CostCenter x) => new(x.Id, x.Code, x.Name, x.IsActive);
    public static DocumentDto ToDto(Document x) => new(x.Id, x.FinancialYearId, x.AccountingPeriodId, x.DocumentType, x.OriginalFileName, x.ContentType, x.SizeInBytes, x.RelatedEntityName, x.RelatedEntityId, x.UploadedAt, x.Notes);
